...
(function (cp) {
    ...
    cp.Vec2.prototype.copy = function () {
        return new cp.Vec2(_this.x, _this.y);
    };
    /**
    ...
    cp.Vec2.prototype.normalize = function () {
        var length = Math.sqrt(_this.length2());
        return new cp.Vec2(_this.x / length, _this.y / length);
    };
    /**
    ...
    cp.Light.prototype.bounds = function () {
        return {
            topleft: new cp.Vec2(_this.position.x - _this.distance, _this.position.y - _this.distance),
            bottomright: new cp.Vec2(_this.position.x + _this.distance, _this.position.y + _this.distance)
        };
    };
    /**
    ...
    cp.Light.prototype.center = function () {
        return new cp.Vec2(_this.distance, _this.distance);
    };
    /**
    ...
    cp.Light.prototype._getVisibleMaskCache = function () {
        ...
        var d = Math.floor(_this.distance * 1.4);
        var hash = '' + d;
        if (_this.vismaskhash != hash) {
            _this.vismaskhash = hash;
            var c = (_this._vismaskcache = createCanvasAnd2dContext('vm' + _this.id, 2 * d, 2 * d));
    ...
    cp.Light.prototype._getHashCache = function () {
        return [_this.distance, _this.diffuse].toString();
    };
    /**
    ...
    cp.Lamp.prototype.center = function () {
        return new cp.Vec2((1 - Math.cos(_this.angle) * _this.roughness) * _this.distance, (1 + Math.sin(_this.angle) * _this.roughness) * _this.distance);
    };
    /**
     ...
     **/
    cp.Light.prototype.bounds = function () {
        var orientationCenter = new cp.Vec2(Math.cos(_this.angle), -Math.sin(_this.angle)).mul(_this.roughness * _this.distance);
        return {
            topleft: new cp.Vec2(_this.position.x + orientationCenter.x - _this.distance, _this.position.y + orientationCenter.y - _this.distance),
            bottomright: new cp.Vec2(_this.position.x + orientationCenter.x + _this.distance, _this.position.y + orientationCenter.y + _this.distance)
        };
    };
    /**
     Render a mask representing the visibility. (Used by DarkMask.)
    ...
    */
    ...
    cp.Neon.prototype.render = function (ctx) {
      ...
      let c = this._getGradientCache(center);
      ctx.drawImage(c.canvas, Math.round(this.position.x-center.x), Math.round(this.position.y-center.y))
    }
    ...
    cp.Neon.prototype._getGradientCache = function (center) {
      let hashcode = this._getHashCache();
      ...
      let D = d*2;
      let cache = createCanvasAnd2dContext(D, D);
      let g = cache.ctx.createRadialGradient(center.x, center.y, 0, d, d, d);
    ...
    */
    ...
    function getTan2(radius, center) {
        ...
        x0, y0, len2, soln, solutions = [], a = radius;
        ...
        var len2a = y0 * Math.sqrt(len2 - a * a), tt = Math.acos((-a * x0 + len2a) / len2), nt = Math.acos((-a * x0 - len2a) / len2), tt_cos = a * Math.cos(tt), tt_sin = a * Math.sin(tt), nt_cos = a * Math.cos(nt), nt_sin = a * Math.sin(nt);
        ...
        if (Math.abs(dist0 - dist1) < epsilon)
            return solutions;
        soln = new cp.Vec2(x0 + nt_cos, y0 - nt_sin);
        solutions.push(soln);
        ...
        if (Math.abs(dist1 - dist2) < epsilon)
            return [soln, solutions[1]];
        if (Math.abs(dist0 - dist2) < epsilon)
            return [solutions[0], soln];
        soln = new cp.Vec2(x0 + tt_cos, y0 + tt_sin);
        solutions.push(soln);
        var dist3 = soln.length2();
        if (Math.abs(dist2 - dist3) < epsilon)
            return [solutions[2], soln];
        if (Math.abs(dist1 - dist3) < epsilon)
            return [solutions[1], soln];
        if (Math.abs(dist0 - dist3) < epsilon)
            return [solutions[0], soln];
    ...
    cp.DiscObject.prototype.cast = function (ctx, origin, bounds) {
        ...
        // let d = new cp.Vec2(originToM.y, -originToM.x).normalize().mul(this.radius);
        // let a = this.center.add(d);
        // let b = this.center.add(d.inv());
        ...
        var distance = (bounds.bottomright.x -
            bounds.topleft.x +
    ...
    cp.DiscObject.prototype.bounds = function () {
        return {
            topleft: new cp.Vec2(_this.center.x - _this.radius, _this.center.y - _this.radius),
            bottomright: new cp.Vec2(_this.center.x + _this.radius, _this.center.y + _this.radius)
        };
    };
    /**
    ...
    cp.PolygonObject.prototype.bounds = function () {
        var topleft = _this.points[0].copy();
        var bottomright = topleft.copy();
        for (var p = 1, l = _this.points.length; p < l; ++p) {
            var point = _this.points[p];
    ...
    cp.RectangleObject.prototype.syncFromTopleftBottomright = function () {
        var a = _this.topleft;
        var b = new cp.Vec2(_this.bottomright.x, _this.topleft.y);
        var c = _this.bottomright;
        var d = new cp.Vec2(_this.topleft.x, _this.bottomright.y);
        _this.points = [a, b, c, d];
    };
    /**
    ...
    var getRGBA = (cp.getRGBA = (function () {
        //let ctx = createCanvasAnd2dContext('grgba', 1, 1);
     ...
     **/
    function extend(extending, arg1, arg2, arg3, arg4) {
