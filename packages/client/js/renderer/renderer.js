...
export default class Renderer {
    constructor(background, entities, foreground, overlay, textCanvas, cursor, game) {
        ...
        // Background
            ...
            self.backContext =
                self.background.getContext('webgl') ||
                self.background.getContext('experimental-webgl');
    ...
    loadSizes() {
        ...
        if (self.webGL) self.map.loadWebGL(self.backContext);
    }
    ...
    drawAnimatedTiles() {
        ...
        if (!self.animateTiles || self.webGL) return;

        self.context.save();
        self.setCameraView(self.context);

        self.forEachAnimatedTile(function (tile) {
            if (!self.camera.isVisible(tile.x, tile.y, 3, 1)) return;
    ...
    drawEntities() {
        ...
        if (self.game.player.dead) return;
        ...
        if (!entity.hasCounter) {
            ...
            if (self.drawNames && entity instanceof Character)
                self.drawText(
                    entity.name,
                    x,
                    self.drawLevels && entity.type !== 'npc' ? y - 8 : y,
                    true,
                    colour,
                    '#000'
                );
    ...
    drawPosition() {
        ...
        self.drawText(
            'x: ' +
                ...
                player.gridY +
                ' tileIndex: ' +
                self.map.gridPositionToIndex(player.gridX, player.gridY),
            10,
            51,
            false,
            'white'
        ...
        if (self.input.hoveringEntity) {
            ...
            self.drawText(
                'attack range: ' + self.input.hoveringEntity.attackRange,
                10,
                91,
                false,
                'white'
    ...
    saveFrame() {
        ...
        if (self.mobile && self.camera.centered) return;
