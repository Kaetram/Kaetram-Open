...
export default class Enchanter {
    ...
    public select(player: Player, index = -1) {
        ...
        if (isNaN(index) || index === -1) return player.notify(t('enchant:CANNOT_ENCHANT'));

        // Grab the slot at the index provided.
        let slot = player.inventory.get(index);

        // Preliminary checks for item validity.
        if (!slot?.key) return player.notify(t('enchant:CANNOT_ENCHANT'));
        ...
        if (slot.count > 1 || !slot.equippable || slot.maxStackSize > 1)
            return player.notify(t('enchant:CANNOT_ENCHANT'));
    ...
    public enchant(player: Player, index = -1, shardIndex = -1): void {
        ...
        if (shardIndex === -1 || index === -1 || isNaN(index) || isNaN(shardIndex))
            return player.notify(t('enchant:NO_ITEM_SELECTED'));
        ...
        if (!itemSlot?.key || !shardSlot?.key) return player.notify(t('enchant:NO_ITEM_SELECTED'));

        // Ensure the player is providing shards to enchant an item.
        if (!shardSlot.key.startsWith('shardt')) return player.notify(t('enchant:NO_SHARD'));
        ...
        if (enchantments.length === 0) return player.notify(t('enchant:NO_ITEM_SELECTED'));
        ...
        if (!chance) player.notify(t('enchant:FAILED_ENCHANT'));
        ...
        if (!this.canEnchant(item, enchantment, level)) player.notify(t('enchant:FAILED_ENCHANT'));
        ...
        player.notify(t('enchant:SUCCESSFUL_ENCHANT'));
